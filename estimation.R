...
DP2LM_estimation <- function(data,length,width,dropout,epoch,batch,directory,lam_length,split,length.ttl,width.ttl,dropout.ttl,epoch.ttl,batch.ttl) {
  ...
  record=list(DE.est=DE.est, IE.est=IE.est, ttl.est= ttl.est, mediator.weights=weights.all, HBIC=HBIC, df=df, t3=t3)
